...
def main():
    ...
    for input_file in range(len(parameters.fastq)):
        ...
        with open("ORFribomap_{}_log.txt".format(riboseq_name), "w") as log:
            ...
            if os.path.exists(mapping_gff): #if the ORFtrack file exists already, ORFtrack is skipped
                ...
                process_orftrack = subprocess.run(orftrack_cmd, shell=True, universal_newlines=True, stdout=subprocess.PIPE)
                log.write(process_orftrack.stdout)
            ...
            if "M" in options:
                ...
                for size in kmer: #mapping of the reads
                    log.write(Mapper.map2bam(size, genome_name, riboseq_name, "all"))
            else:
